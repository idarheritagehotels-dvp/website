...
class ADODB_Pager {
    ...
    var 10; // number of links per page in navigation bar
    var $showPageLinks;

	var 'width=100% border=1 bgcolor=white';
	...
	var 'Page';
	var 'red';
	var 0;  #secs to cache with CachePageExecute()
	...
	//
	function __construct(&$db,$sql,'adodb', false)
	{
	global $PHP_SELF;

		$id.'_curr_page';
		if (!empty($PHP_SELF)) htmlspecialchars($_SERVER['PHP_SELF']); // htmlspecialchars() to prevent XSS attacks
	...
	// original code by "Pablo Costa" <pablo@cbsp.com.br>
        ...
        global $PHP_SELF;
            ...
            $this->rs->LastPageNo();
            $this->linksPerPage ? $this->linksPerPage : $pages;
            for(1; $i <= $pages; $i+=$linksperpage)
            {
			...
			if ($this->startLinks && $start > 1) {
				$start - 1;
				$numbers .= "<a href=$PHP_SELF?$pos>$this->startLinks</a>  ";
            }
			...
			for($start; $i <= $end; $i++) {
                if ($this->rs->AbsolutePage() == $i)
                    $numbers .= "<font color=$this->linkSelectedColor><b>$i</b></font>  ";
                     ...
                     $numbers .= "<a href=$PHP_SELF?$i>$i</a>  ";

            }
			if ($this->moreLinks && $end < $pages)
				$numbers .= "<a href=$PHP_SELF?$i>$this->moreLinks</a>  ";
            print $numbers . ' &nbsp; ';
        }
	// Link to previous page
	function render_prev(true)
	{
	global $PHP_SELF;
		if ($anchor) {
	?>
	...
	global $ADODB_COUNTRECS;
		...
		$ADODB_COUNTRECS;
		if ($this->db->pageExecuteCountRows) true;
		if ($this->cache)
	...
	function RenderLayout($header,$grid,$footer,'border=1 bgcolor=beige')
	{
